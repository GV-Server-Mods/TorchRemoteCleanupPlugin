# Torch Plugin Development Guide: Remote Grid Abandon

A complete step-by-step guide to building and deploying **Remote Grid Abandon**, a Torch server plugin that overrides Space Engineers' native **Terminal -> Info Tab** grid removal ("X" button) to safely destroy player beacons and clear ownership rather than deleting the entire grid, featuring a dark-themed Torch WPF control panel, persistent XML configuration, real-time telemetry, dedicated log auditing, and in-game admin chat commands.
...
| File | Purpose |
| :--- | :--- |
| [`TorchRemoteCleanupPlugin.csproj`](TorchRemoteCleanupPlugin.csproj) | Modern SDK-style project file configured for .NET Framework 4.8 (`net48`, `x64`) with `<UseWPF>true</UseWPF>` enabled for WPF markup compilation. Builds `RemoteAbandon.dll`. |
| [`manifest.xml`](manifest.xml) | Plugin metadata descriptor for Torch (`Remote Grid Abandon`, version 2.0.0). |
| [`Plugin.cs`](Plugin.cs) | Main entry point inheriting `TorchPluginBase` and implementing `IWpfPlugin`. Manages persistent XML configuration (`Persistent<RemoteAbandonConfig>`), statistics, and lifecycle. |
| [`Config/RemoteAbandonConfig.cs`](Config/RemoteAbandonConfig.cs) | Observable ViewModel configuration class containing all plugin settings with `[Display]` annotations. |
| [`Services/RemoteAbandonStatistics.cs`](Services/RemoteAbandonStatistics.cs) | Thread-safe real-time telemetry model tracking grids abandoned, subgrids handled, beacons destroyed, PCU refunded, and last grid info. |
| [`Utils/ChatUtils.cs`](Utils/ChatUtils.cs) | Helper for safely dispatching in-game notifications to players via `ModCommunication.SendMessageTo`. |
| [`Commands/RemoteAbandonCommands.cs`](Commands/RemoteAbandonCommands.cs) | In-game chat command module under `[Category("abandon")]` providing `status`, `stats`, `resetstats`, `toggle`, `reload`, `set`, and `info`. |
| [`Views/RemoteAbandonControl.xaml`](Views/RemoteAbandonControl.xaml) | WPF user control designed with the dark theme matching `GridDefender`. Contains the **Configuration** and **Live Telemetry** tabs. |
| [`Views/RemoteAbandonControl.xaml.cs`](Views/RemoteAbandonControl.xaml.cs) | Code-behind for the WPF view handling save confirmation and statistics reset. |
| [`RemoteAbandonPatch.cs`](RemoteAbandonPatch.cs) | Harmony prefix patch on `MyBlockLimits.RemoveBlocksBuiltByID` executing the customizable abandon workflow and writing audit logs. |
...
Open [`TorchRemoteCleanupPlugin.csproj`](TorchRemoteCleanupPlugin.csproj) and verify `<TorchDir>`:
```xml
